...
`create` returns `id` when Home Assistant is asked for an action response. A notification without a `key` appends to the feed; the same `key` replaces its existing item while retaining its generated `id`.
